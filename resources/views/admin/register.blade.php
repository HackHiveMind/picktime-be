        ...
        <section class="relative mx-auto grid w-full max-w-5xl self-center overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-xl shadow-neutral-950/10 lg:grid-cols-[0.95fr_1.05fr]">
            <div class="bg-neutral-950 p-6 text-white sm:p-8">
                ...
                <div class="mt-16 max-w-sm">
                    <p class="text-sm font-medium text-[#74bd45]">First setup</p>
                    <h1 class="mt-3 text-3xl font-semibold leading-tight">Create first admin</h1>
                    <p class="mt-4 text-sm leading-6 text-neutral-300">Set up the first protected account for the iHUB booking administration workspace.</p>
                </div>
            </div>

            <form method="POST" action="/admin/register" class="p-6 sm:p-8 lg:p-10">
                @csrf
                <div class="mb-8">
                    <p class="text-sm font-semibold text-[#74bd45]">Protected setup</p>
                    <h2 class="mt-2 text-2xl font-semibold">Admin account</h2>
                </div>

                <div class="grid gap-5">
                    <label class="grid gap-2 text-sm font-medium">Name
                        <input class="h-12 rounded-md border border-neutral-300 bg-white px-4 outline-none transition focus:border-[#74bd45] focus:ring-2 focus:ring-[#74bd45]/20" name="name" value="{{ old('name') }}" required>
                    </label>
                ...
                <button class="mt-8 h-12 w-full rounded-md bg-neutral-950 px-4 font-semibold text-white transition hover:bg-[#74bd45] hover:text-neutral-950 focus:outline-none focus:ring-2 focus:ring-[#f7de05] focus:ring-offset-2" type="submit">Register</button>
            </form>
        </section>
    </main>
</body>
